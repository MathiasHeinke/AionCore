...
use aionui_ai_agent::{
    CommandEveAsyncCompletionDispatch, CommandEveAsyncCompletionResult, IWorkerTaskManager, types::BuildTaskOptions,
};
...
impl IAsyncCompletionTurnRunner for ConversationService {
    async fn run(
        ...
        self.run_command_eve_async_completion_turn(request, turn_id, project_build_options)
            .await
    }
}
...
impl CommandEveAsyncCompletionConsumer {
    ...
    async fn consume(&self, dispatch: &CommandEveAsyncCompletionDispatch) -> CommandEveAsyncCompletionResult {
        ...
        let outcome = match tokio::time::timeout(self.turn_timeout, turn).await {
            ...
            Err(_) => {
                ...
                if !self.mark_unknown(dispatch, &payload_sha256, "turn_timeout").await {
                    return retryable("ack_persistence_failed");
                }
        ...
        match outcome {
            Err(ConversationError::Busy { .. }) => {
                    ...
                    .await
                {
                    Ok(true) => {
                        ...
                        retryable("conversation_busy")
                    }
                    Ok(false) | Err(_) => {
                        if !self
                            .mark_unknown(dispatch, &payload_sha256, "outcome_unknown_retry_transition")
                            .await
                        {
                            return retryable("ack_persistence_failed");
                        }
                        unknown("outcome_unknown_retry_transition")
                    }
                }
            }
            Ok(outcome) if outcome.status == ConversationAgentTurnStatus::Completed => {
                    ...
                    .await
                {
                    ...
                    Ok(false) | Err(_) => {
                        if !self
                            .mark_unknown(dispatch, &payload_sha256, "complete_transition_failed")
                            .await
                        {
                            return retryable("ack_persistence_failed");
                        }
            ...
            Ok(_) => {
                if self.mark_unknown(dispatch, &payload_sha256, "turn_failed").await {
            ...
            Err(error) => {
                ...
                if self.mark_unknown(dispatch, &payload_sha256, "turn_error").await {
    ...
    async fn mark_unknown(
        ...
        if let Err(error) = self
            .receipt_repo
            .mark_unknown(&dispatch.request.completion_id, &self.owner_instance_id, code)
            .await
        {
...
mod tests {
    ...
    async fn consumer_persists_accepted_and_fails_closed_when_the_ack_writer_refuses_success() {
        ...
        let (dispatch, _receiver) = dispatch("session-1");
        assert_eq!(
            consumer.consume(&dispatch).await,
            CommandEveAsyncCompletionResult::Completed {
                turn_id: "turn-accepted".to_owned()
            }
        );
    ...
    async fn consumer_persists_already_applied_in_flight_retryable_and_unknown_claims() {
        let cases = [
            ...
            (
                ...
                CommandEveAsyncCompletionResult::Unknown {
        ...
        for (claim, expected_result, expected_status, expected_code) in cases {
            ...
            let (dispatch, _receiver) = dispatch("session-1");
            assert_eq!(consumer.consume(&dispatch).await, expected_result);
    ...
    #[tokio::test]
    async fn post_bind_foreign_session_is_durably_rejected_while_pre_bind_remains_retryable() {
        let rejected_repo = Arc::new(RecordingReceiptRepo::new(AsyncCompletionReceiptClaim::Conflict));
        let (consumer, runner) =
            consumer_with(rejected_repo.clone(), StubTurnResult::Completed, Some("session-bound")).await;
        let (dispatch, _receiver) = dispatch("session-foreign");
        assert_eq!(
            consumer.consume(&dispatch).await,
            CommandEveAsyncCompletionResult::Rejected {
        ...
        assert_eq!(runner.calls.load(Ordering::SeqCst), 0);
        assert_eq!(
            rejected_repo.recorded_rejections(),
            vec![RecordedRejection {
                completion_id: "completion-1".to_owned(),
                conversation_id: "conversation-1".to_owned(),
                bound_acp_session_id: "session-bound".to_owned(),
        ...
        let (dispatch, _receiver) = dispatch("session-pending-load");
        assert_eq!(
            consumer.consume(&dispatch).await,
            CommandEveAsyncCompletionResult::RetryableBusy {
        ...
        let (consumer, _) = consumer_with(refusing_repo, StubTurnResult::Completed, Some("session-bound")).await;
        let (dispatch, _receiver) = dispatch("session-foreign");
        assert_eq!(
            consumer.consume(&dispatch).await,
            CommandEveAsyncCompletionResult::RetryableBusy {
